...
set BUILD_DEST_LIB_DIR=%BUILD_DIR%\lib\RelWithDebInfo
...
if not exist "%BUILD_DEST_LIB_DIR%\" mkdir "%BUILD_DEST_LIB_DIR%"
...
cmake %SOURCE_DIR% -A x64 -DCMAKE_CONFIGURATION_TYPES=RelWithDebInfo

:: 如果你想要直接构建（使用MSBuild），可以添加以下行  
:: msbuild MyProject.sln /p:Configuration=Release /p:Platform="x64"  


set DIR_OPENCV_LIB=%DIR_3RDPARTY_LIB%\opencv\windows
xcopy "%DIR_OPENCV_LIB%\*" "%BUILD_DEST_LIB_DIR%" /E /I /Y
